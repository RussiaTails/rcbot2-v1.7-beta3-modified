...
bool CBotTF2 :: canGotoWaypoint (Vector vPrevWaypoint, CWaypoint *pWaypoint, CWaypoint *pPrev)
{
	if ( CBot::canGotoWaypoint(vPrevWaypoint,pWaypoint,pPrev) )
	{
		...
		/*if (pWaypoint->hasFlag(CWaypointTypes::W_FL_AMMO) && (CTeamFortress2Mod::isMapType(TF_MAP_SAXTON) && m_iClass == TF_CLASS_HWGUY))
		{
			return false;
		}*/
...
void CBotTF2 :: getTasks ( unsigned int iIgnore )
{
	...
	ADD_UTILITY(BOT_UTIL_GETFLAG, (CTeamFortress2Mod::isMapType(TF_MAP_CTF)||CTeamFortress2Mod::isMapType(TF_MAP_TC)||CTeamFortress2Mod::isMapType(TF_MAP_RD) ||CTeamFortress2Mod::isMapType(TF_MAP_CP)||(CTeamFortress2Mod::isMapType(TF_MAP_SD)&&CTeamFortress2Mod::canTeamPickupFlag_SD(iTeam,false))) && !bHasFlag,fGetFlagUtility)
...
bool CBotTF2 :: handleAttack ( CBotWeapon *pWeapon, edict_t *pEnemy )
{
	...
	if ( pWeapon )
	{
		...
		/*if (CTF2Conditions::TF2_IsPlayerInCondition(engine->IndexOfEdict(pEnemy), TFCond_HalloweenKart))
		{
			setMoveTo(CBotGlobals::entityOrigin(pEnemy));
			//setLookAt(m_vAimVector);
			setLookAtTask(LOOK_ENEMY);
		...
		if (CTeamFortress2Mod::isMapType(TF_MAP_SAXTON) && m_iClass == TF_CLASS_HWGUY)
		{
				setMoveTo(CBotGlobals::entityOrigin(pEnemy));
				setLookAtTask(LOOK_ENEMY);
				m_fAvoidTime = engine->Time() + 1.0f;
...
bool CBotTF2 :: isEnemy ( edict_t *pEdict,bool bCheckWeapons )
{
	...
	if ( CBotGlobals::isPlayer(pEdict) )
	{
		if ( CBotGlobals::getTeam(pEdict) != getTeam())
		{
			...
			return false; // Don't attack invisible players

			if (CTF2Conditions::TF2_IsPlayerInCondition(engine->IndexOfEdict(pEdict), TFCond_HalloweenKart))
			{
