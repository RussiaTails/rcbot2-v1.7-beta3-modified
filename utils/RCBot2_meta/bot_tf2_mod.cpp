...
void CTeamFortress2Mod :: mapInit ()
{
	...
	if (std::strncmp(szmapname, "ctf_", 4) == 0 || std::strncmp(szmapname, "quake_turbine", 13) == 0 || std::strncmp(szmapname, "pass_", 5) == 0 || std::strncmp(szmapname, "pd_", 3) == 0 || std::strncmp(szmapname, "od_", 3) == 0 || std::strncmp(szmapname, "slendytubbies", 13) == 0 && (!std::strncmp(szmapname, "ctf_chouhen_b2", 14) == 0 || !std::strncmp(szmapname, "ctf_haarp", 9) == 0 )) // Quake Turbine is CTF. - RussiaTails
		m_MapType = TF_MAP_CTF; // capture the flag
	else if (std::strncmp(szmapname, "cp_", 3) == 0 || std::strncmp(szmapname, "cqt_", 4) == 0 || std::strncmp(szmapname, "conquest_", 9) == 0 || std::strncmp(szmapname, "dom_", 4) == 0 || std::strncmp(szmapname, "2koth_", 6) == 0 || std::strncmp(szmapname, "ctf_chouhen_b2", 14) == 0 || std::strncmp(szmapname, "ctf_haarp", 9) == 0 || std::strncmp(szmapname, "falling_cp", 10) == 0 || std::strncmp(szmapname, "stt_", 4) == 0 || std::strncmp(szmapname, "vip_", 4) == 0 && (!std::strncmp(szmapname, "cp_helmsdeep_v2", 15) == 0 )) // Conquest, 2koth and DOM works fine as CP_. Moved stt to cp to make bots attack a tank - RussiaTails
		m_MapType = TF_MAP_CP; // control point
	else if (std::strncmp(szmapname, "tc_", 3) == 0 )
		m_MapType = TF_MAP_TC; // territory control
	else if (std::strncmp(szmapname, "pl_", 3) == 0 || std::strncmp(szmapname, "tow_", 4) == 0 && (!std::strncmp(szmapname, "pl_fountain", 11) == 0 || !std::strncmp(szmapname, "pl_divulgence", 13) == 0 ))// Tug of War works fine as Payload - RussiaTails
...
{
	return (!iTeam || getEnemyTeam(iTeam) == getTeam(pEntity)) && std::strcmp(pEntity->GetClassName(),"item_teamflag")==0 || std::strcmp(pEntity->GetClassName(), "passtime_ball") == 0;
}
...
bool CTeamFortress2Mod::isBoss(edict_t* pEntity, float* fFactor)
{
		...
		isMapType(TF_MAP_CP) || isMapType(TF_MAP_PD) || isMapType(TF_MAP_ARENA) || isMapType(TF_MAP_SAXTON) || isMapType(TF_MAP_SD) || isMapType(TF_MAP_DM))
	{
		...
		if (std::strcmp(pEntity->GetClassName(), "base_boss") == 0 || // For Krampus and any other NPS which uses base_boss entity - RussiaTails
